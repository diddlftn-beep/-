...
# [수정됨] key 값을 t1_, t2_, t3_ 로 완전히 분리하여 충돌 방지

# --- 제품 1 ---
with tab1:
    p1_name = st.text_input("제품명 (1)", placeholder="예: 옥스포드 셔츠", key="t1_name")
    p1_cost = st.number_input("원가 (1)", value=None, step=1000, format="%d", key="t1_cost")
...
# --- 제품 2 ---
with tab2:
    p2_name = st.text_input("제품명 (2)", placeholder="예: 데님 팬츠", key="t2_name")
    p2_cost = st.number_input("원가 (2)", value=None, step=1000, format="%d", key="t2_cost")
    col1, col2, col3 = st.columns(3)
...
# --- 제품 3 (여기가 문제였음 - ID 전면 교체) ---
with tab3:
    p3_name = st.text_input("제품명 (3)", placeholder="예: 니트 베스트", key="t3_name")
    p3_cost = st.number_input("원가 (3)", value=None, step=1000, format="%d", key="t3_cost")
    col1, col2, col3 = st.columns(3)
